//============================================================================//
//=  DTVGM.h                                                               ===//
//=  Distributed Time-Variant Gain Model: MONTHly Scale                    ===//
//=                                                                        ===//
...
double DTVGM_Month_MCMC(int nBasinGrid,int ngrid,TDate dateBegin,TDate dateEnd,
                              String strDirRain,String strPrefix_rainfile,
                              int nBagrovN,double *ArrayofBagrovN,double *gridBagrovN,
                              int *gridSubID,double *gridArea,double *gridRCC,
                              double *Array_ETp,double *RunoffObs,
...
//        BasinArea = Parameter[4];      //这里不用该参数
        ...
        for(i=0;i<nMonths;i++) //处理iMonth个月的数据
        {
                ...
                { //Runoff Generation
                        ...
                        gridETa[k] = ETp*dlETatoETp;//((1 - KAW)*dlETatoETp + KAW*sqrt(gridsm0[k]/AWC));   // 2003-10-30 (1 - KAW)*

                        //Kr = Kr0*(1.0 + dlgridrain[k]/300.0); *exp(2.5*dlgridrain[k]/300.0)
                        ...
                        if(fabs(gridRCC[k] - 1.0) < 1.0e-3) //不考虑影响因子
                                ...
                                facym = (gridRCC[k] + 0.01)*(1.0 + facmon[imonth - 1]);//gridRCC[k] + facmon[imonth - 1];  //年因子＋月因子
...
/*
        ...
        for(i=0;i<nMonths;i++) //处理iMonth个月的数据
        {
                ObjectFunction += (sqrt(RunoffObs[i])-sqrt(RunoffCal[i]))*(sqrt(RunoffObs[i])-sqrt(RunoffCal[i]));
        }
*/
        delete[] gridrain;
...
{       double retNR,dsum;
        ...
        for(int i=0;i<12;i++)
        {
                dsum += random(1.0e4)/1.0e4;
        }
        retNR = miu + sigma*(dsum - 6.0);
        return retNR;
} //end
//==============================================================
